...
- **Admin login:** Supabase email/password at `website/admin/login.html` — **no TOTP/MFA** (the H5 two-factor rollout was reverted per Angel's decision in `155be5f`, 2026-06-17; `requireAuth()` in auth.js only checks for a session and reveals owner-only UI via `is_owner`). The login form carries a Cloudflare **Turnstile** CAPTCHA (same site key as the public form; token passed as `captchaToken` to `signInWithPassword` and `resetPasswordForEmail`) — enforced server-side **only when** Supabase Auth → Settings → Bot & Abuse Protection is ON with the Turnstile secret; until then the token is sent and ignored. Authorisation is the 6-email `is_admin()` allowlist + RLS (above), plus Supabase's built-in auth rate limits.
