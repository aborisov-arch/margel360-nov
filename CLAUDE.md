...
- **Admin login:** Supabase email/password at `website/admin/login.html`, hardened with **two factors**: a Cloudflare **Turnstile** CAPTCHA (token passed as `captchaToken` to `signInWithPassword`; must be turned on in Supabase Auth → Settings → Bot & Abuse Protection with the Turnstile secret to be enforced server-side) **and TOTP MFA**. `requireAuth()` (auth.js) enforces **AAL2** on every admin page — a password-only (AAL1) session with no factor is sent to `website/admin/security.html` to enrol; one with an un-passed factor is bounced to login for the 6-digit code. **Lost device:** an owner removes the user's factor in Supabase dashboard → Authentication → Users → (user) → Factors; the admin re-enrols on next login.
...
| **Weekday promo (TEMPORARY, ends 2026-08-31)** | 20% off **venue base only**, events **Mon–Thu** with date ≤ 2026-08-31; beats promo codes (code NOT burned when promo wins) | submit-enquiry `WEEKDAY_PROMO` (**authoritative**), reservation.js `WEEKDAY_PROMO` (summary mirror + date hint), main.js promo-bar expiry date, promo.html (copy + price table), translations-*.js `promo_bar_*` keys. **After 31 Aug:** remove the bar + promo.html (301 to /), strip both WEEKDAY_PROMO blocks |
