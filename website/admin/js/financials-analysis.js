...
document.addEventListener('click',event=>{const b=event.target.closest('[data-pay-month]');if(!b)return;monthFilter=b.dataset.payMonth;document.getElementById('fin-month').value=monthFilter;document.getElementById('fin-month').dispatchEvent(new Event('change'));closeDrill();openManagerPay();});
...
document.addEventListener('submit',async event=>{
  ...
  try{const {error}=await db.from(table).upsert(payload,{onConflict:table==='manager_monthly_pay'?'month,manager_email':'event_id,manager_email'});if(error)throw error;payrollDrafts.delete(savedKey);await loadManagerPay();renderMonthSummary();renderEventOvertime(currentSelection()?.fe);showToast('Запазено.');}catch(error){showToast('Записът не успя. Проверете стойностите и връзката.','error');button.disabled=false;}
});
