...
| Rule | Value | Files |
|---|---|---|
| Venue base prices | evening 1350 (raised from 1280 on 2026-08-18 — consumer fallback maps deliberately keep 1280 to price pre-stamp legacy rows at their quoted price), corp4 330, corp8 440, bday_day 700, bday_eve 970, wedding 1500 | **Legacy-NULL fallback only** — the effective venue price is computed by `_shared/seasonal-pricing.ts` and stamped into `enquiries.venue_price_eur` at submit and on preferred_date edits; reservation-catalog.js, enquiry-email.ts, dashboard.js, financials.js, customers.js, marketing.js, offer-export.js (EVENT_CONFIG), offer-pdf.js (PDF_EVENT_CONFIG) keep these maps only to price pre-stamp rows (`venue_price_eur IS NULL`) |
